...
import TrendinCard from "@/components/TrendinCard";
              ...
              {trendingMovies && 
                <View className="mt-10">
                  ...
                  <FlatList data={trendingMovies} renderItem={({item, index}) => (
                    <TrendinCard movie={item} index = {index}/>
                  )} keyExtractor={(item) => item.movie_id.toString()} className="mt-3 mb-4 pb-18" horizontal showsHorizontalScrollIndicator = {false} ItemSeparatorComponent={() => <View className="w-4"/>}/>
                </View>
              }
              <>
